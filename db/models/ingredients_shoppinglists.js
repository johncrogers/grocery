module.exports.insertIngredients_Shoppinglists = (data) => {
  const conn = require('../conn.js').conn;
  console.log(`  -> Inserting ingredients_shoppinglists...`);
  return conn('ingredients_shoppinglists').insert(data).then(() => {
...
module.exports.selectIngredients_Shoppinglists = (query, columns) => {
  const conn = require('../conn.js').conn;
  console.log(`  -> Retrieving ingredients_shoppinglists...`);
  columns ? console.log(`  -> using:`, columns) : columns = '*';
  if (query) {
    return conn.select(columns).from('ingredients_shoppinglists').where(query).then((rows) => {
  ...
  return conn.select(columns).from('ingredients_shoppinglists').then((rows) => {
    console.log(`  -> Successfully pulled row data.`);
    return rows;
  }).catch((err) => {
    console.log(`ERROR: An error occurred when retrieving ingredients_shoppinglists.`, err);
